...
Covers:
  ...
  - Hierarchy: admin > SecEngineer > viewer
...
from core.api.permissions import IsAdmin, IsSecEngineer, IsViewer, _has_role, require_role
...
def test_viewer_passes_viewer_check(db, groups):
    ...
    assert _has_role(user, "SecEngineer") is False
    assert _has_role(user, "admin") is False


def test_secengineer_inherits_viewer(db, groups):
    user = _make_user("u2", role="SecEngineer", groups=groups)
    assert _has_role(user, "viewer") is True
    assert _has_role(user, "SecEngineer") is True
    assert _has_role(user, "admin") is False


def test_admin_inherits_secengineer_and_viewer(db, groups):
...
@require_role("SecEngineer")
def _secengineer_view(request):
    return HttpResponse("ok")


def test_require_role_redirects_anonymous(client, db):
    response = client.get("/__test_secengineer_view__/")
    ...
    response = _secengineer_view(request)
...
# ── DRF IsViewer/IsSecEngineer/IsAdmin permission classes ────────────
...
def test_is_viewer_grants_for_all_roles(db, groups):
    ...
    perm = IsViewer()
    rf = RequestFactory()
    for role in ("viewer", "SecEngineer", "admin"):
        request = rf.get("/x/")
...
def test_is_admin_grants_only_for_admin(db, groups):
    ...
    perm = IsAdmin()
    rf = RequestFactory()
    for role in ("viewer", "SecEngineer"):
        request = rf.get("/x/")
...
def test_is_secengineer_denies_viewer(db, groups):
    from django.test import RequestFactory
    perm = IsSecEngineer()
    rf = RequestFactory()
    request = rf.get("/x/")
    request.user = _make_user("ise_v", role="viewer", groups=groups)
    assert perm.has_permission(request, view=None) is False
...
def test_setup_rbac_creates_three_groups(db):
    Group.objects.filter(name__in=["viewer", "SecEngineer", "admin"]).delete()
    call_command("setup_rbac")
    names = set(Group.objects.values_list("name", flat=True))
    assert {"viewer", "SecEngineer", "admin"}.issubset(names)


def test_setup_rbac_is_idempotent(db):
    call_command("setup_rbac")
    call_command("setup_rbac")  # second run must not fail
    names = list(Group.objects.filter(name__in=["viewer", "SecEngineer", "admin"]).values_list("name", flat=True))
    assert sorted(names) == ["SecEngineer", "admin", "viewer"]
